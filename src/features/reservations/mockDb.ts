import { BOOKABLE_UNITS, getUnitLabels } from '@/features/properties/mockProperties'
...
const generateMocks = (): Reservation[] => {
  ...
  for (let i = 1; i <= 45; i++) {
    ...
    reservations.push({
      ...
      propertyId: BOOKABLE_UNITS[i % BOOKABLE_UNITS.length].id,
...
export const DB = generateMocks()

export const simulateGetReservations = async (
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (params.search) {
        ...
        filtered = filtered.filter(
          (res) =>
            ...
            getUnitLabels(res.propertyId).property.toLowerCase().includes(lowerSearch),
        )
      }
      ...
      resolve({
        data: paginatedData,
        meta: { totalCount, totalPages, currentPage: safePage, limit: params.limit },
      })
    }, 500)
  })
}
